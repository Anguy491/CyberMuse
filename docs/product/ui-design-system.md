...
- 专业模式中，准确使用中性高对比；偏高使用上箭头、垂直位置和“偏高”，偏低使用下箭头、垂直位置和“偏低”。轻松模式由独立呈现层使用“目标内 / 接近目标 / 向上或向下调整 / 高于或低于目标”，±25 cents 内不得以“偏高/偏低”作为主状态；signed cents 和方向箭头继续存在。数值使用 tabular 数字，宽度变化不得推动布局。
