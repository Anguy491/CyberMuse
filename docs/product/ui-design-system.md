...
- Shell 由稳定导航和页面内容组成；Library/Settings 始终存在，当前歌曲/session 使 Practice/Review 出现。活动 Practice 不隐藏已有导航项，选择其他页面时由退出门禁延迟提交路由；当前歌曲或长任务状态只在相关页面就地显示。
...
- `primary`：练习主列按“稳定录唱状态、Pitch Lane、中部反馈舞台、底部控制坞”占用页头以下视口。Pitch Lane 保持实时图形焦点且不因右侧歌词高度拉伸；其下方的弹性留白是刻意保留的反馈舞台，不使用卡片、纹理、阴影或装饰。
- `secondary`：transport 的视觉与键盘顺序为播放/暂停图标、回到开头图标、开始/重试录唱、A-B Loop、练习数据、专业模式 switch、原唱 switch 和按需恢复动作；播放不会自动开启麦克风。操作区、时间与 seek 共同锚定练习主列底部；有歌词时只占左侧主列。A-B Loop 与三组练习指标默认收起为两个 44×44 px 图标入口，并互斥向上覆盖反馈舞台；展开层使用不透明 canvas、受控高度和内部滚动，不改变控制坞位置。专业模式 switch 紧邻“练习数据”右侧，原唱 switch 紧邻专业模式右侧，两者都包含可见“开/关”。
- 稳定状态行最左侧使用 44×44 px“?”入口展开线型图例；麦克风进入 ready 后以绿色圆点和“正在录唱”双重编码状态，即使尚未收到稳定音高也不退回“等待录唱”，方向和数值不得进入该状态。figure accessible name 与中部反馈舞台使用同一节流后摘要且不设高频 live region。开始/重试录唱先显示模态说明与错误详情，继续动作才调用既有权限流程。
...
- Reference Track 使用互不连接的实心校准刻度：每个刻度聚合约 80–120 ms 参考音高，外层低对比厚度表达 ±50 cents、内层较高对比厚度表达 ±25 cents、中心使用短实心横线；不得绘制连续外框、虚线边界或连接相邻刻度。当前用户轨使用带 P10/P90 包络与极值点的实线，最近 take 使用灰色点线，无参考检测使用独立未评分点线。半音网格强化 C/八度线并标音名；专业模式视窗外用顶/底三角 overflow 表达，不把曲线钳在边缘。灰度/forced-colors 下通过厚度、间隔、线型、形状和按需图例区分。
...
- 中部反馈舞台的主提示复用页面 display 尺寸，目标/当前音名和 signed cents 使用较小的 tabular 辅行。专业模式中，准确使用中性高对比；偏高使用上箭头、垂直位置和“偏高”，偏低使用下箭头、垂直位置和“偏低”。轻松模式由独立呈现层使用“目标内 / 接近目标 / 向上或向下调整 / 高于或低于目标”，±25 cents 内不得以“偏高/偏低”作为主状态；signed cents 和方向箭头继续存在。数值宽度变化不得推动控制坞或 Pitch Lane。
...
- 轻松模式的整体 signed bias 在 ±25 cents 内使用“目标区内”句式并保留高/低方向的精确信息，不使用“整体偏高/偏低”；Review 始终提供“练习整首”，持续错误区间再提供“重新练习此处”。
...
- Pitch Lane 与输入电平不套用上述过渡，继续由 AudioContext 时间和 30–60 FPS UI 节流驱动；大字号反馈文本使用首条立即、后续 300 ms、短暂无声 600 ms 的离散呈现节奏。两者都不得用 CSS tween 补帧或反向影响音频主时钟。
- 反馈等级继续遵守 NFR-005 的滞回限制；提示、音名和 cents 必须从同一呈现快照原子替换，不得触发布局抖动。
