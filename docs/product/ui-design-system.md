...
Dark 与 light 是同等的一等主题，均必须覆盖全部组件、页面状态、视觉回归和可访问性门禁。M1 可以先以 dark 建立实现和截图基线，但这只是交付顺序，不降低 light 的完成标准。默认主题跟随 Windows 系统设置；除非 FR-020 后续明确加入主题偏好，不新增持久化设置。
...
1. `primary`：用户此刻应读懂或操作的唯一焦点单元，使用最大字号/最高对比/最多留白，不放入卡片。
2. `secondary`：完成任务所需的上下文、控件、列表或解释，密度可变但不得与 primary 等权。
3. `tertiary`：导航、技术元信息、稳定状态和辅助动作，使用 label/caption，不得形成第四套强调。

- 设计基线为 1280×800；必须在 1024×720 和 Windows 150% 缩放下保持主流程可操作。
- 桌面导航使用水平文字导航和清晰 active indicator，不使用悬浮图标岛或移动端 tab bar。导航属于 tertiary。
- Shell 由稳定导航、全局状态和页面内容组成；当前歌曲、分析任务、离线/模型状态可扫描但不抢占 primary。
...
- `tertiary`：歌曲/session、设备、保存状态和 accessible legend。
...
- `tertiary`：歌曲/session、时间、数据覆盖说明和导出/诊断辅助动作。
...
- 状态标签可以使用紧凑英文阶段码，但面向用户的错误与恢复动作必须可本地化。

## 实现与评审检查表

- 页面和组件只使用语义 token；dark/light 都有定义，新增 token 先更新本文并说明用途。
- 每屏恰好三层、一个 primary 焦点、一个 deliberate pattern break、最多两个字体家族/三个字号/两个字重。
