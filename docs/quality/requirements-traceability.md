...
| FR-005 | Analyzer outputs；AnalysisManifest；ADR-020/021 | TC-AN-002 产物/哈希/schema/原子提交；TC-AN-005 stem/F0 语义有效性 | M4/M6 | `test_pipeline.py`、`test_semantic_validation.py` 与 Rust store 覆盖媒体/哈希/原子提交及 collapsed stem 拒绝；HTDemucs 两首指定真实歌曲通过，20 首/人工语义矩阵仍待 M6 补齐；[M4 evidence](../delivery/evidence/m4-offline-analyzer.md)、[M6 evidence](../delivery/evidence/m6-review-and-release.md) |
...
| FR-013 | Audio scoring；Data metrics；Signal UI feedback semantics；ADR-012 | TC-SCO-001 cents/等级/滞回；TC-FBK-001 轻松文案；TC-FBK-002 首条/300 ms/600 ms/硬边界 | M3/M10 | `packages/scoring/src/scoring.test.ts`、`use-readable-feedback.test.ts`、Practice/Review page tests；评分边界保持 M3，可读呈现层见 [M10 evidence](../delivery/evidence/m10-ux-closeout.md) |
...
| FR-018 | UX Review；Signal UI Review hierarchy/Metric；SessionMetrics | TC-REV-001 摘要/损坏范围/重练区间；TC-FBK-001/TC-NAV-001 轻松摘要与整首重练 | M6/M10 | `session_store.rs` 局部损坏 salvage；`review-model.test.ts` 和 `ReviewPage.test.tsx` 覆盖摘要、不可用范围、错误区间、整首/区间重练；[M10 evidence](../delivery/evidence/m10-ux-closeout.md) |
...
| FR-020 | AppSettings API；UX Settings；ADR-017 | TC-SET-001 保存/修订冲突/设备失效；TC-NAV-001 Practice→主题连续 revision | M6/M10 | `settings_store.rs` 默认/恢复/修订冲突/校准，`app-settings.schema.json`；Audio Settings 并发 revision 队列、真实 input/output 恢复、默认设备 group 变化可见回退、显示偏好/音量，Practice 设备身份与主题共用 `PreferencesProvider` revision 队列，Model Assets exact cache 同步 UI tests；[M6 evidence](../delivery/evidence/m6-review-and-release.md)、[M10 evidence](../delivery/evidence/m10-ux-closeout.md) |
...
| FR-027 | Scoring mode；Pitch Lane index/model；Practice/Review；ADR-023；M10 calibration-tick refinement | TC-PLV2-001..004、TC-SES-002、TC-A11Y-002、TC-PLV3-001、TC-FBK-001 | M8/M10 | M8 scoring/controller/session tests 保持；M10 lane/page/review/i18n/style tests 与 [M10 evidence](../delivery/evidence/m10-ux-closeout.md)；发布视觉人工矩阵开放 |
...
| FR-029 | UX 信息架构；typed App route；Practice exit guard | TC-NAV-001 路由上下文/整首与区间重练/保存门禁/异步顺序 | M10 | `app-route-machine.test.ts`、`App.test.tsx`、`PracticePage.test.tsx`、`ReviewPage.test.tsx` 与 [M10 evidence](../delivery/evidence/m10-ux-closeout.md) |
...
| NFR-005 | Audio smoothing/Pitch Lane FPS；300/600 ms 文本呈现；Signal UI motion；ADR-012 | TC-PERF-003 30–60 FPS/等级抖动；TC-FBK-002 可读反馈时序 | M3/M10 | M3 有效 UI 46.9 FPS、500 ms 等级往返 0；M10 受控时钟 hook/page/style tests；[M3 evidence](../delivery/evidence/m3-fixture-practice.md)、[M10 evidence](../delivery/evidence/m10-ux-closeout.md) |
...
| NFR-008 | Analyzer pipeline；ADR-013/014/020/021 | TC-ANPERF-001 3/5/10 分钟 CPU-only；TC-MOD-002 正确性优先 bake-off | M4/M6 | HTDemucs 的 234 秒真实歌曲完整 pipeline 为 120.987 秒；M4 的 Spleeter P95/RAM/临时空间/包体证据不可继承，新模型仍须重跑 3/5/10 分钟矩阵并满足或显式修订预算；[M6 evidence](../delivery/evidence/m6-review-and-release.md) |
...
| NFR-016 | Supply-chain integrity；ADR-013/016/020/021 | TC-SUP-001 locks/hash/SBOM/licenses；TC-MOD-002 候选/生产分层 | M4/M5/M6 | HTDemucs model release 的 MIT LICENSE/NOTICE、lineage、URL/size/SHA 已入清单，Spleeter/TensorFlow 从生产构建移除；旧 1,060-file/SPDX/installer 数字已失效，必须对新的单-sidecar 产物重跑供应链与 release assets；[M6 evidence](../delivery/evidence/m6-review-and-release.md) |
| NFR-017 | Accessible feedback；Signal UI dual-theme contrast/focus/motion/font fallback | TC-A11Y-001 dark/light/键盘/灰度/forced-colors/焦点/reduced-motion/断网字体回退；TC-FBK-001/002、TC-NAV-001 | M1/M3/M6/M10 | `signal-ui.test.ts`、Practice/Review/App page tests；M10 校准刻度、可读反馈舞台、底部控制坞和完整导航见 [M10 evidence](../delivery/evidence/m10-ux-closeout.md) |
...
| NFR-023 | ADR-023；Pitch Lane index/LOD；M10 calibration ticks/独立文本节奏；三层音高真值 | TC-PERF-005 4 ms P95/30–60 FPS；TC-PIT-002 三层精度；TC-PLV3-001；TC-FBK-002 | M8/M10 | 自动坐标/降采样/刻度间距、性能与文本节奏隔离证据；合法 vocal-stem、校准硬件回环和发布显示矩阵仍按 [M8 evidence](../delivery/evidence/m8-pitch-lane-v2.md) 开放；M10 呈现见 [M10 evidence](../delivery/evidence/m10-ux-closeout.md) |
...
- TC-AN-QUALITY-001 的 M4 历史基线实测 SwiftF0 voiced recall 1.0、gross/octave error 0、median 4.111 cents、unvoiced false positive 0；Spleeter 合成 fixture 的 vocal/instrumental SI-SDRi 12.860/15.406 dB、重建 -35.251 dB。该 Spleeter 数据不再代表 ADR-021 当前生产模型。
...
- FR-005/006/019、NFR-008/016：ADR-021 将分离路径替换为 exact `demucs-htdemucs@spectral-v1.0.0`，Rust catalog/request 与 Python ONNX tensor contract 均锁定 209,469,333 bytes 和 SHA-256 `c3395410…3d02`；模型只由 consented downloader 安装，analyzer 仍不联网。TC-AN-005 在 F0/manifest 前拒绝逐 PCM 相同或比例塌缩的可听 stems，并有静音、相同、非相同单元测试。
- 用户指定的两首完整歌曲通过 `m6_demucs_real_song_report.py`：`Ceremony` stem correlation 0.102311、重构残差 0.034043、voiced ratio 0.364956；`Moth To A Flame` 为 0.066979、0.022034、0.829060。两者均有不同的 PCM24 vocals/instrumental、非空 reference track、三项 artifact SHA-256；匿名报告位于 Git 忽略的 `artifacts/m6/demucs-real-songs/verification-report.json`。
- 该两歌结果满足用户指定的初步 Demucs 验收，但不替代 TC-AN-005 的 20 首/6–8 首真值 stems finalist 矩阵，也不继承旧模型 TC-ANPERF-001、clean-host、断网/取消和人工试听证据；这些项继续阻止 M6 退出。
...
- 用户于 2026-08-29 明确批准 M8 gate 后完成 Tauri 双 capability、共享 AudioContext 双 media 图、30 ms vocal gain、“原唱”switch、独立 fallback/retry 及 session/scoring 隔离。`pnpm check`、Rust fmt/clippy/test、受控 10 分钟/loop/故障矩阵、既有 NFR 性能回归和 Tauri release build 通过。用户随后明确确认 M9 门禁并授权 M10；Windows 私有真实歌曲耳听、显示/硬件矩阵和实际 WebView2 长时资源采样作为已接受限制继续保留，执行状态见 [M9 Original Vocal Guide Evidence](../delivery/evidence/m9-original-vocal-guide.md)。

## M10 实现证据

- FR-013/018/027/029 与 NFR-003/005/017/023 的实现、自动命令和剩余人工显示矩阵统一记录在 [M10 UI and Navigation Closeout Evidence](../delivery/evidence/m10-ux-closeout.md)。M10 不修改 scoring package、session schema、Tauri/API、AudioWorklet/Worker 或 analyzer；若验证发现这些边界有差异，M10 立即停止申请门禁。
