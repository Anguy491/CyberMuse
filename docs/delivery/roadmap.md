...
M7 交付用户自备 LRC 的行级解析、偏移校准、逐句导航和 Practice 右侧同步显示；不联网获取歌词，不做自动转录或逐词评价。M8 升级 Pitch Lane 的目标通道、乐句稳定刻度、像素桶异常保真和可逆的专业/八度折叠评分，并以三层精度证据而非视觉观感判定正确性。M9 在不改动评分/session 的前提下，把现有伴奏与原唱 stems 接入同一播放图，提供默认关闭、页面内即时切换的原唱辅助，并以双轨同步、故障降级和资源预算作为门禁。M10 完成产品收尾：将连续虚线目标通道改为独立实心校准刻度，为轻松模式增加不改变评分的支持性反馈文案，并以类型化路由和统一退出门禁补全 Library/Practice/Review/Settings 导航。后续候选仍包括节奏 onset/duration、DTW、0.75× 慢速、多 take 叠加和移调；进入规划前分别验证变速音频质量与时序评分不会把表达性偏差误判。
