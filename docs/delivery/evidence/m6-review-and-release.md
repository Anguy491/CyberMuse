...
| 日期 | 2026-08-26 |
| 责任域 | sessions、Review、calibration、settings、diagnostics、Windows release |
| 需求依据 | FR-015、FR-017、FR-018、FR-020、FR-021；NFR-001、NFR-012..017、NFR-019..021 |
| 候选 commit | `c2e259dec9c5c02f2d6251a85a7ecc6925acf835`（构建时工作树 dirty） |
...
- Audio Settings 保存设备 fingerprint、音量、主题、reduced motion、exact model cache 和按输入/输出组合的自动/手动延迟校准；语义默认设备绑定当前 group identity，Practice 取得权限并恢复实际输入、解析实际输出和共享采样率后才应用完全匹配的校准，设备或采样率变化时可见回退零补偿。
...
| 命令/用例 | 结果 |
|---|---|
| `pnpm check` | PASS：format、lint、strict typecheck；22 个 unit 文件/126 项；3 个 contract 文件/14 项；47 条直接依赖审批与 285 个 Windows Cargo 包 |
| `cargo fmt --all -- --check` | PASS |
| `cargo clippy --workspace --all-targets -- -D warnings` | PASS |
| `cargo test --workspace` | PASS：49 项 |
...
- Size：391,966,944 bytes
- SHA-256：`6c50deebd936b90755c18142a03a574e5f294d28a32ad351758893f112eea3b5`
- 签名：无；工作树：dirty；clean-host：未满足；`distributionAllowed=false`。
- SPDX：301 个依赖包加 CyberMuse 根包，共 302 个 package entries、196,982 bytes，SHA-256 `70a64eb9753bc237c79f98757ae78af923c7f88c7690d656ade60bec995dd854`。
- Notices：706,057 bytes，SHA-256 `40f0dff29d1a5cabd714dc0bc659390088f72e35fb8d969afd309dd084f4c22b`。
- Model manifest：SHA-256 `c860c70d82280c95edada53d83c8a8c882eeef4495243c9ad858de70b8646b13`。
...
5. 完成 dark/light、100%/150%、1024×720/1280×800、键盘、灰度、forced-colors、reduced-motion 和真实歌曲反馈语义的人工矩阵。
